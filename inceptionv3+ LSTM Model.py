...
# # inceptionv3-LSTM Video Classification Model

# In[7]:


def inceptionv3_lstm_model():
    # Load the pre-trained InceptionV3 model without the top (fully connected) layers
    pretrained_model = InceptionV3(include_top=False, weights='imagenet', input_shape=(IMAGE_HEIGHT, IMAGE_WIDTH, 3))
    ...
    # Apply the pre-trained InceptionV3 to each frame in the sequence
...
# Create the InceptionV3-LSTM model
model = inceptionv3_lstm_model()
...
# Save your Model.
model.save("InceptionV3_model3.keras")
