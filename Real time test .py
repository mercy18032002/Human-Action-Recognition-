...
total_correct_predictions = sum(label_correct_counts.values())
total_samples = sum(label_total_counts.values())

# Calculate total accuracy
total_accuracy = (total_correct_predictions / total_samples) * 100
...
# Load the saved model for testing
loaded_model3 = tf.keras.models.load_model("Xception_model4.h5")
...
for i in range(len(features_test)):
    ...
    # Make predictions using the trained model for the current sample
    predicted_label = np.argmax(loaded_model3.predict(np.expand_dims(features_test[i], axis=0))[0])
...
# Get the predicted labels for the test features
predicted_labels = np.argmax(loaded_model3 .predict(features_test), axis=1)
